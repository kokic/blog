...
UIMode.Connect = class extends UIMode {
    constructor(source, forged_vertex, reconnect = null) {
        super();
    ...
    static valid_connection(reconnect = null) { }
...
class UI {
    ...
    initialise() {
        ...
        this.canvas = new DOM.Div({}, canvas_style).add_to(this.container);
    ...
    reposition_focus_point(update_focus_position = true) {
    ...
    static dismiss_error(type = null) {
        const error = new DOM.Element(document.body).query_selector(`.error-banner${type !== null ? `[data-type="${type}"]` : ""
            }`);
        if (error) {
            const SECOND = 1000;
...
class History {
    ...
    /// real-time.
    add(ui, invoke = false, selection = ui.selection) {
...
class Panel {
    ...
    initialise(ui) {
        ...
        const display_export_pane = (modify = (output) => output) => { };
    ...
    create_option_list(
        ...
        const create_option = (value, tooltip, data) => {
            ...
            if (options.style.name.startsWith("corner")) {
                button.set_style({
                    "background-image": ["", "un"].map((prefix) => {
                        return `url("icons/${options.style.name.endsWith("inverse") ? "var-" : ""
            ...
            for (const colour of ["black", "grey"]) {
                ...
                backgrounds.push(`url("data:image/svg+xml;utf8,${encodeURIComponent(svg.element.outerHTML)}")`);
            }
                    ...
                    if (cell.options.style.name === "arrow") {
                        for (const component of ["tail", "body", "head"]) {
                            ...
                            switch (cell.options.style[component].name) {
                                ...
                                case "hook":
                                case "harpoon":
                                    value = `${cell.options.style[component].side
...
class Vertex extends Cell {
    ...
    render(ui) {
        ...
        if (construct) {
            ...
            // this.element.set_style({ position: 'absolute' });
